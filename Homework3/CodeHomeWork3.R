...
getEntropy <- function(class.freq){
  ...
  # *** ATTENTION !!! *** #
  entropy <- apply(p,1, function(x) {-x*log(x,base=(2))} )
...
getInfoGain <- function(split_var, class_var){
  ...
  attr.class.freq <- table(split_var, class_var)
  ...
  for(i in 1:nrow(attr.class.freq)){
    ...
    # *** ATTENTION !!! *** #
    child.node.freq <- as.matrix((attr.class.freq[i,]))
    ...
    # *** ATTENTION !!! *** #
    all_obs_parent.node <- sum(apply(attr.class.freq,2,sum))
    entropy_split <- entropy_split + child.node.entropy*(num_obs_child.node/all_obs_parent.node )
    entropy_split
  ...
  for(i in 1:number_of_columns){
    ...
    if(class(data.X[,i]) == "numeric"){
      ...
      for(j in 1:(length(x_sort_unique)-1)){
        ...
        # *** ATTENTION !!! *** #
        cutoff_point <- (x_sort_unique[i]+x_sort_unique[i+1]) / 2
        ...
        # *** ATTENTION !!! *** #
        info_gain_for_this_split <- getInfoGain(lessthan_cutoff,data.X[,i])
...
data.X = perceptron.data[,1:2]
data.Y = perceptron.data[,3]
...
hypothesis <- as.matrix(w)  %*%  t(as.matrix(X.new))
dim(hypothesis)
...
# *** ATTENTION !!! *** #
k.fold.data.idx <- createFolds(heart.disease.db$class, k = 10)
