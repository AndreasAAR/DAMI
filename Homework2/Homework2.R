...
setwd( "/Users/AAR/Dropbox/Andreas/PLUGG_JOBB/Autumn19/DAMI/Homework2")
load("cluster.RData")
...
k_means <- function(x, k, max.iter = 20){
  ...
  while(TRUE){
    ...
    for(i in 1:k){
      ...
      centroids[i,] <- colMeans(x[obs_of_cluster_i,], dims = 1)
    }
...
# The elbow method is argued to use the "at-the-bend" position,
# to get a rough estimate of were the sum of square tapers of, 
# and the clusters stop describing any useful variance.
# Id argue that 3 is the correct grouping. 
...
kmed.cl <- pam(cluster.data,3,metric = "manhattan")

# plot points
# show medoids in the plot
points(kmed.cl$medoids)
...
plot(x = 1:4, y = sil_info$sil, type='b', xlab = "number of centers K", ylab = "Avg Sillhouette width" )

# Based on the table above, what value of K gives the best result
# [WRITE YOUR ANSWER HERE]
#The max silhouette width is most desirable when choosing clusters, here its 4.
...
distance_matrix <- dist(small.dataset)

# Call a built-in R function to perform hierarchical clustering
hierarchial.clust <- hclust(distance_matrix)
